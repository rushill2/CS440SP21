...
class TabQPolicy(QPolicy):
    ...
    def qvals(self, states):
        ...
        """
        qvals = np.zeros((1,2))
        disc_state = self.discretize(states[0])
        for i in range(0, 2):
            if i == 0:
                qvals[0][i] = self.model[disc_state + (0,)]
            elif i == 1:
                qvals[0][i] = self.model[disc_state + (1,)]
            # elif i == 2:
            #     qvals[0][i] = self.model[disc_state + (2,)]
        return qvals

    def td_step(self, state, action, result, next_state, done):
        """
        One step TD update to the model

        @param state: the current state
        @param action: the action
        @param result: the result of taking the action at the current state
        ...
        q_vals = self.model[disc_state+ (action,)]
        disc_next = self.discretize(next_state)
        
        if done:
            result = 1.0
            goal = result
        else:
            max_ = max(self.model[disc_next+ (0,)], self.model[disc_next+ (1,)])
            goal = result
            goal += self.gamma*max_

        self.model[disc_state + (action,)] = q_vals + self.lr*(goal - q_vals)
        loss = (q_vals - goal)
        loss = loss**2
        return loss
